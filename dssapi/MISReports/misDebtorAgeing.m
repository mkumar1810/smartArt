...
@implementation misDebtorAgeing
...
- (id) initDebtorAgeingReportWithFrame:(CGRect) frame forOrientation:(UIInterfaceOrientation) p_intOrientation
{
    ...
    if (self) {
       ...
       // [[NSNotificxationCenter defaultCenter] addObserver:self selector:@selector(reportCoreDataGenerated:)  name:@"MISDebtorAgeingGenerated" object:nil];
        [navTitle setTitle:@"Debtor Ageing"];
...
- (void) generateDataForOffset:(int) p_addOffset
{
    ...
    {
        orderColumn = [[NSString alloc] initWithFormat:@"%@",@"TOTAL"];
        isAscending = NO;
        ...
        [[dssWSCallsProxy alloc] initWithReportType:@"MISDEBTORAGEING" andInputParams:inputDict andReturnMethod:l_proxyReturn andReturnInputs:NO];
    }
}
...
- (void) reportCoreDataGenerated:(NSDictionary *)generatedInfo
{
    ...
    NSLog(@"received data %@", dataForDisplay);
