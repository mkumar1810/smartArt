...
@implementation misFGTransferred
...
- (id) initReportForDate:(NSString*) p_fordate andDayOffset:(int) p_dayoffset andFrame:(CGRect) frame forOrientation:(UIInterfaceOrientation) p_intOrientation
{
    ...
    if (self) {
        ...
        //[[NSNotificaxtionCenter defaultCenter] addObserver:self selector:@selector(reportCoreDataGenerated:)  name:@"MISFGTransferGenerated" object:nil];
        [navTitle setTitle:@"Finished Goods Transferred"];
...
- (void) generateDataForOffset:(int) p_addOffset
{
    if (populationOnProgress==NO)
    {
        ...
        NSDictionary *inputDict = [[NSDictionary alloc] initWithObjectsAndKeys:[NSString stringWithFormat:@"%d", _offset],@"p_dayoffset",@"MIS_FGTransferredToStore",@"p_reporttype",  nil];
        METHODCALLBACK l_proxyReturn = ^(NSDictionary* p_dictInfo)
        {
            [self reportCoreDataGenerated:p_dictInfo];
        };
        [[dssWSCallsProxy alloc] initWithReportType:@"MISFGTRANSFER" andInputParams:inputDict andReturnMethod:l_proxyReturn andReturnInputs:NO];
    }
}
    ...
    if(cell == nil) 
    {
        ...
        for (int _lblNo = 0; _lblNo<5; _lblNo++) 
        {
            ...
            switch (_lblNo) {
                ...
                case 2:
                    xWidth = colWidth;
                    strTitle = [[NSString alloc] initWithFormat:@"%@   ",[frm stringFromNumber:[NSNumber numberWithInteger:[[p_dataDict valueForKey:@"PRODON"] integerValue]]]];
                    lblBackColor = [UIColor colorWithRed:205.0f/255.0f green:133.0f/255.0f blue:63.0f/255.0f alpha:1.0];
                    break;
                case 3:
                    xWidth = colWidth;
                    strTitle = [[NSString alloc] initWithFormat:@"%@   ",[frm stringFromNumber:[NSNumber numberWithInteger:[[p_dataDict valueForKey:@"PRODUP"] integerValue]]]];
                    lblBackColor = [UIColor colorWithRed:205.0f/255.0f green:133.0f/255.0f blue:63.0f/255.0f alpha:1.0];
                    break;
                case 4:
                    xWidth = colWidth;
                    strTitle = [[NSString alloc] initWithFormat:@"%@   ",[frm stringFromNumber:[NSNumber numberWithInteger:[[p_dataDict valueForKey:@"ASOFTODAYPROD"] integerValue]]]];
                    lblBackColor = [UIColor colorWithRed:205.0f/255.0f green:133.0f/255.0f blue:63.0f/255.0f alpha:1.0];
                    break;
                default:
                    break;
