...
@implementation misSalesCollection
...
- (id) initReportForDate:(NSString*) p_fordate andDayOffset:(int) p_dayoffset andFrame:(CGRect) frame forOrientation:(UIInterfaceOrientation) p_intOrientation
{
    ...
    if (self) {
        ...
        //[[NSNotificxationCenter defaultCenter] addObserver:self selector:@selector(reportCoreDataGenerated:)  name:@"MISSalesCollectionGenerated" object:nil];
        [navTitle setTitle:@"Sales Collection"];
...
- (void) generateDataForOffset:(int) p_addOffset
{
    if (populationOnProgress==NO)
    {
        ...
        NSDictionary *inputDict = [[NSDictionary alloc] initWithObjectsAndKeys:[NSString stringWithFormat:@"%d", _offset],@"p_dayoffset",@"MIS_SalesCollection",@"p_reporttype",  nil];
        METHODCALLBACK l_proxyReturn = ^(NSDictionary* p_dictInfo)
        {
            [self reportCoreDataGenerated:p_dictInfo];
        };
        [[dssWSCallsProxy alloc] initWithReportType:@"MISSALESCOLLECTION" andInputParams:inputDict andReturnMethod:l_proxyReturn andReturnInputs:NO];
    }
}
...
- (UITableViewCell*) getDetailCell:(UITableView*) p_dispTV andData:(NSDictionary*) p_dataDict forRowNo:(int) p_rowNo
{
    ...
    int colWidth, xPosition, slWidth, smWidth, maxRecordNo, xWidth, colHeight;;
    maxRecordNo = [dataForDisplay count] ;
    ...
    if(cell == nil) 
    {
        ...
        for (int _lblNo = 0; _lblNo<8; _lblNo++) 
        {
            ...
            switch (_lblNo) {
                ...
                case 2:
                    xWidth = colWidth;
                    strTitle = [[NSString alloc] initWithFormat:@"%@ ",[frm stringFromNumber:[NSNumber numberWithInteger:[[p_dataDict valueForKey:@"COLLECTIONON"] integerValue]]]];
                    lblBackColor = [UIColor colorWithRed:205.0f/255.0f green:133.0f/255.0f blue:63.0f/255.0f alpha:1.0];
                    break;
                case 3:
                    xWidth = colWidth;
                    strTitle = [[NSString alloc] initWithFormat:@"%@ ",[frm stringFromNumber:[NSNumber numberWithInteger:[[p_dataDict valueForKey:@"COLLECTIONUP"] integerValue]]]];
                    lblBackColor = [UIColor colorWithRed:205.0f/255.0f green:133.0f/255.0f blue:63.0f/255.0f alpha:1.0];
                    break;
                case 4:
                    xWidth = colWidth;
                ...
                case 5:
                    xWidth = colWidth;
                    strTitle = [[NSString alloc] initWithFormat:@"%@ ",[frm stringFromNumber:[NSNumber numberWithInteger:[[p_dataDict valueForKey:@"COLLECTIONLASTMONTH"] integerValue]]]];
